...
/-- The phase of a position in the breathing cycle. -/
...
/-- Classify each position into its phase. -/
def CyclePos.phase (p : CyclePos) : Phase :=
...
theorem expansion_count :
    (Finset.univ.filter (fun p : CyclePos => p.val < 6)).card = 6 := by
  decide
...
theorem contraction_count :
    (Finset.univ.filter (fun p : CyclePos => p.val ≥ 6)).card = 7 := by
  decide
...
theorem checkpoint_spacing : ∀ i : Fin 4,
    [3, 6, 9, 12].get ⟨i.val, by omega⟩ = 3 * (i.val + 1) := by
  intro i
  fin_cases i <;> simp
